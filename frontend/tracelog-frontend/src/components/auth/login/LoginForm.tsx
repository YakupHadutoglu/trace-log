  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', padding: '24px 64px', position: 'relative', zIndex: 1, height: '100vh', overflow: 'hidden', justifyContent: 'space-between' }}>
      ...
      <div style={{ maxWidth: '380px', margin: '0 auto', width: '100%', display: 'flex', flexDirection: 'column' }}>
        <span style={{ width: '6px', height: '6px', borderRadius: '50%', background: '#10B981', display: 'inline-block', marginBottom: '16px' }}></span>
        <h1 style={{ fontSize: '26px', fontWeight: 800, letterSpacing: '-1.2px', margin: '0 0 6px', color: '#FFFFFE' }}>Sign in to TraceLog</h1>
        <p style={{ fontSize: '14.5px', color: '#A1A1AA', margin: '0 0 24px' }}>Observability for every database you run.</p>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <div>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '7px' }}>
              <label style={{ fontSize: '12.5px', fontWeight: 600, color: '#D4D4D8' }}>Password</label>
              <Link href="/forgot-password" style={{ fontSize: '12.5px', color: '#A1A1AA', textDecoration: 'none' }}>Forgot password?</Link>
            </div>
            <input type="password" placeholder="••••••••••••" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} style={inputStyle} />
          </div>

          <button type="submit" style={{ background: '#FE5203', color: '#FFFFFE', fontSize: '14.5px', fontWeight: 600, padding: '12px', borderRadius: '7px', border: 'none', cursor: 'pointer', marginTop: '4px' }}>
            Sign in
          </button>
        </form>

        <p style={{ fontSize: '13.5px', color: '#A1A1AA', textAlign: 'center', marginTop: '24px' }}>
          Don`t have an account?{' '}
          <Link href="/register" style={{ color: '#FE5203', fontWeight: 600, textDecoration: 'none' }}>Create one</Link>
        </p>
      </div>
      ...
      <p style={{ fontSize: '11.5px', color: '#525258', textAlign: 'center' }}>
        ...
        {' '}·{' '}
        <Link href="#" style={{ color: '#525258', textDecoration: 'none' }}>Privacy</Link>
      </p>
    </div>
  )
}
